...
class HodgkinHuxleyNeuron:
    def __init__(self, dt=0.01, C_m=1.0, neuron_id=0, refractory_period=2.0):
        ...
        self.E_L = -54.387
        self.V = -65.0
        self.m = self.alpha_m(self.V) / \
            (self.alpha_m(self.V) + self.beta_m(self.V))
        self.h = self.alpha_h(self.V) / \
            (self.alpha_h(self.V) + self.beta_h(self.V))
        self.n = self.alpha_n(self.V) / \
            (self.alpha_n(self.V) + self.beta_n(self.V))
    ...
    def update(self, I_ext=0.0):
        ...
        if not in_refractory:
            ...
            self.last_V = self.V
            self.V += dV
            self.m += self.dt * (self.alpha_m(self.V) *
                                 (1 - self.m) - self.beta_m(self.V) * self.m)
            self.h += self.dt * (self.alpha_h(self.V) *
                                 (1 - self.h) - self.beta_h(self.V) * self.h)
            self.n += self.dt * (self.alpha_n(self.V) *
                                 (1 - self.n) - self.beta_n(self.V) * self.n)
    ...
    def has_spiked(
        self): return self.last_V < self.spike_threshold and self.V >= self.spike_threshold

    def just_spiked(
        self): return self.last_V < self.spike_threshold and self.V >= self.spike_threshold

    def is_refractory(self): return (
        self.t - self.last_spike_time) < self.refractory_period

    def reset(self): self.__init__(
        self.dt, self.C_m, self.id, self.refractory_period)
...
class NeuronalNetwork:
    def __init__(self, num_neurons, dt=0.01, T=50.0, refractory_periods=None):
        ...
        if refractory_periods is None:
            ...
            raise ValueError(f"refractory_periods length ({len(
                refractory_periods)}) must match num_neurons ({num_neurons})")

        self.neurons = [HodgkinHuxleyNeuron(
            dt=dt, neuron_id=i, refractory_period=refractory_periods[i]) for i in range(num_neurons)]

        # Connectivity: weight[i, j] means (j -> i)
        self.weights: list[tuple[float, float]] = np.zeros(
    ...
    def set_current_for_neuron(self, neuron_idx, t_start, t_end, amplitude):
        self.I_ext[neuron_idx, int(t_start / self.dt)
                                   :int(t_end / self.dt)] = amplitude
    ...
    def draw_architecture(self, layer_assignment=None):
        ...
        nx.draw(G, pos, with_labels=True, node_color='skyblue',
                node_size=1000, arrows=True, arrowstyle='->', arrowsize=20)
        edge_labels = nx.get_edge_attributes(G, 'weight')
        nx.draw_networkx_edge_labels(G, pos, edge_labels={(
            i, j): f'{w:.0f}' for (i, j), w in edge_labels.items()})
        ...
        plt.savefig(os.path.join(
            output_dir, 'hodgkin_huxley_network_architecture.png'), dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def plot_results(self):
        ...
        for i, neuron in enumerate(self.neurons):
            plt.plot(self.time, neuron.get_voltage_history(),
                     label=f'Neuron {i}')
        ...
        os.makedirs(output_dir, exist_ok=True)
        plt.savefig(os.path.join(
            output_dir, 'hodgkin_huxley_network_dynamics.png'), dpi=300, bbox_inches='tight')
        plt.close()
...
def run_hodgkin_huxley_network():
    ...
    net.set_current_for_neuron(
        neuron_idx=0, t_start=0.0, t_end=50.0, amplitude=10.0)
    net.set_current_for_neuron(
        neuron_idx=1, t_start=10.0, t_end=50.0, amplitude=10.0)
    net.set_current_for_neuron(
        neuron_idx=2, t_start=20.0, t_end=50.0, amplitude=10.0)
