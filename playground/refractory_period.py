...
def run_refractory_period():
    ...
    for i, ref_period in enumerate(refractory_periods):
        ...
        # voltages / spikes
        line1 = ax1.plot(neuron.get_voltage_history(),
                         color=colors[i], linewidth=2, label='Voltage')
        ax1.set_ylabel('Voltage (mV)', color=colors[i])
        ax1.tick_params(axis='y', labelcolor=colors[i])
        ax1.grid(True, alpha=0.3)

        # applied current
        line2 = ax2.plot(I_test, color='black', linewidth=1.5,
    ...
    plt.savefig(os.path.join(output_dir, 'refractory_period.png'),
